...
<template>
  <div class="grid grid-row-6  min-h-full py-4 dark:text-white">
    ...
    <template v-if="!showSignUpEmailPassword">
      <div
        class="row-span-4 bg-blue-50 dark:bg-gray-800 dark:text-white relative place-self-center overflow-hidden flex flex-col gap-2 w-80  pb-12 px-2 glass rounded-lg"
      >
        ...
        <div class="flex flex-row-reverse justify-center gap-1 mb-4">
                    <div @click="handleGoToLogin" class="text-primary  cursor-pointer">
                      {{ t('auth.loginIn') }}
                    </div>
                    <div>  {{ t('auth.goLogin') }}</div>
        </div>
